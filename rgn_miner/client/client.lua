...
Citizen.CreateThread(function()
    ...
    SetBlipSprite(blip, -1633216922, 1)
        Citizen.InvokeNative(0x9CB1A1623062F402, blip, "Mining Job") 
end)
        ...
        if (Vdist(pos.x, pos.y, pos.z, Config.Zones['init'].x, Config.Zones['init'].y, Config.Zones['init'].z) < 3.0) then
            if Mine1 == false then
                ...
                if IsControlJustPressed(0, 0xC7B5340A) then
                    TriggerServerEvent('rgn_miner:startjob')
                end
            end
        end
        if FirstMine == true then
            ...
            if (Vdist(pos.x, pos.y, pos.z, Config.Zones['Mine1'].x, Config.Zones['Mine1'].y, Config.Zones['Mine1'].z) < 3.0) then
                ...
                if IsControlJustPressed(0, 0xC7B5340A) then
					...
					FirstMine = false
                    Mine2 = true
					TriggerServerEvent('rgn_miner:additem')
                end
            end
        ...
        elseif LastMine == true then
            ...
            if (Vdist(pos.x, pos.y, pos.z, Config.Zones['Vehicle'].x, Config.Zones['Vehicle'].y, Config.Zones['Vehicle'].z) < 3.0) then
                ...
                if IsControlJustPressed(0, 0xC7B5340A) then
                    ...
                    TriggerEvent('rgn_miner:lastroute')
                    RemoveBlip(blip5)
            ...
            if (Vdist(pos.x, pos.y, pos.z, Config.Zones['Delivery'].x, Config.Zones['Delivery'].y, Config.Zones['Delivery'].z) < 3.0) then
                ...
                if IsControlJustPressed(0, 0xC7B5340A) then
                    if IsPedInAnyVehicle(PlayerPedId(), true) then
						...
						RemoveBlip(blip6)
						TriggerServerEvent('rgn_miner:pay', money, xp)
						TriggerServerEvent('rgn_miner:removeitem')
                        Delivery = false
                        Mine1 = false
                    else
...
RegisterNetEvent('rgn_miner:lastroute')
AddEventHandler('rgn_miner:lastroute', function()
	Citizen.Wait(0)
...
RegisterNetEvent('rgn_miner:start')
AddEventHandler('rgn_miner:start', function(source)
...
RegisterNetEvent('rgn_miner:timer')
AddEventHandler('rgn_miner:timer', function()
	local timer = 220
